  ...
  return (
    <View style={[styles.root, light && styles.rootLight]} testID="native-pokedex-detail-screen">
      <ScrollView contentContainerStyle={[styles.content, { paddingTop: 8 + insets.top, paddingBottom: 100 + insets.bottom }]} keyboardShouldPersistTaps="always" nestedScrollEnabled>
        ...
        <View onLayout={motion.onLayout} testID="native-pokedex-detail-workspace-motion">
          <NativeHorizontalPageSlider activeIndex={TABS.findIndex(([value]) => value === tab)} onIndexChange={(index) => setTab(TABS[index]?.[0] ?? 'registered')} scrollX={motion.scrollX} sizing="content" swipeEnabled={false}>
        <View style={[styles.tabPanel, { minHeight: Math.max(0, height - insets.top - 70) }]} testID="native-pokedex-detail-panel-registered">
        ...
        <View style={[styles.tabPanel, { minHeight: Math.max(0, height - insets.top - 70) }]} testID="native-pokedex-detail-panel-info">
        ...
        <View style={[styles.tabPanel, { minHeight: Math.max(0, height - insets.top - 70) }]} testID="native-pokedex-detail-panel-battle">
        ...
        <View style={[styles.tabPanel, { minHeight: Math.max(0, height - insets.top - 70) }]} testID="native-pokedex-detail-panel-more">
...
const styles = StyleSheet.create({
  ...
  tabs: { flexDirection: 'row', borderWidth: 1, borderColor: '#34424a', borderRadius: 14, backgroundColor: '#141a1f' }, tab: { position: 'relative', minHeight: 55, flex: 1, alignItems: 'center', justifyContent: 'center' }, tabText: { color: '#aeb9bf', fontSize: 11, fontWeight: '900', textTransform: 'uppercase' }, tabDetail: { maxWidth: '90%', color: '#85939a', fontSize: 7, fontWeight: '800' }, tabTextActive: { color: '#fff' }, tabTextActiveLight: { color: '#14232a' }, tabIndicator: { height: 4, borderRadius: 3 }, tabPanel: { gap: 14 },
